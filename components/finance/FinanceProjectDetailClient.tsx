...
function Overview({ detail, isGlobal, candidates, onMutate }: { detail: FinanceProjectDetail; isGlobal: boolean; candidates: ReturnType<typeof useFinanceMemberCandidates>; onMutate: (path: string, method: "POST" | "PUT", body: unknown, success: string) => Promise<void> }) {
  ...
  return <div className="finance-stack">
    ...
    <section className="card finance-section"><div className="card-head"><h2>재무 담당자</h2><span className="hint">첫 번째 담당자가 주담당자입니다.</span></div>
      ...
      {isGlobal && <div className="finance-edit-block"><label className="lab">재무 담당자</label>{candidates.candidates.length ? <><select className="select" multiple value={managerIds} onChange={(event) => setManagerIds(Array.from(event.currentTarget.selectedOptions, (option) => option.value))}>{candidates.candidates.map((candidate) => <option key={candidate.crewMemberId} value={candidate.crewMemberId}>{candidate.name}</option>)}</select><label className="lab">주담당자</label><select className="select" value={primaryManagerId} onChange={(event) => setPrimaryManagerId(event.target.value)}>{managerIds.map((crewMemberId) => { const candidate = candidates.candidates.find((row) => row.crewMemberId === crewMemberId); return <option key={crewMemberId} value={crewMemberId}>{candidate?.name ?? "담당자"}</option>; })}</select></> : <p className="hint">{candidates.unavailable ? "담당자 선택 목록을 불러오지 못했습니다. 새로고침 후 다시 시도해주세요." : "담당자 선택 목록을 불러오는 중입니다."}</p>}<p className="hint">여러 명을 선택할 수 있으며 주담당자를 별도로 지정합니다.</p><button className="btn" type="button" disabled={busy || !candidates.candidates.length} onClick={() => void saveManagers()}><UsersRound size={14} /> 담당자 저장</button></div>}
    </section>
  </div>;
}
