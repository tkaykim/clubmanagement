...
import { financeBudgetSchema } from "@/lib/finance";
...
export async function PUT(request: Request, { params }: Params) {
  ...
  const parsed = financeBudgetSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return financeApiError(400, "VALIDATION_ERROR", "예산 입력값이 올바르지 않습니다", { details: parsed.error.issues });
  const supabase = createRouteSupabaseClient();
  const { error } = await supabase.rpc("finance_upsert_project_budget", {
