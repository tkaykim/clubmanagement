...
export async function POST(request: Request, { params }: Params) {
  ...
  if (!parsed.success) return financeApiError(400, "VALIDATION_ERROR", "확정 입력값이 올바르지 않습니다", { details: parsed.error.issues });
  const supabase = createRouteSupabaseClient();
  const { error } = await supabase.rpc("finance_confirm_allowance_batch", {
