...
import { financePaymentSchema } from "@/lib/finance";
...
export async function POST(request: Request, { params }: Params) {
  ...
  const parsed = financePaymentSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return financeApiError(400, "VALIDATION_ERROR", "지급 입력값이 올바르지 않습니다", { details: parsed.error.issues });
  const supabase = createRouteSupabaseClient();
  const { error } = await supabase.rpc("finance_record_payment", {
